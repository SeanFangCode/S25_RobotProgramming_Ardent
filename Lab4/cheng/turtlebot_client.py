...
class TurtleClient(Node):
	...
	#### Display/Turtle Setup ####
        ...
        ##added new parameters for command line inputs#########
        self.declare_parameter('turtleColor', 'red', ParameterDescriptor(description= 'Color of the turtle'))
        self.declare_parameter('penSize', 1, ParameterDescriptor(description= 'size of the pen'))
        #######################################################
	
	##added code for parameter for turtleColor and penSize#
        ...
        self.color_req.color = self.get_parameter('turtleColor').value
...
def main(args=None):
    ...
    while rclpy.ok():
        ...
        ###cli_obj.twist_pub.publish(cmd_msg)## commended out to ignore server overwrite
